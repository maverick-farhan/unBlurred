@extends('main_layout')
@section('dynamic')
    <h1 class="editor text-2xl font-extrabold font-condensed text-center">Update: POST EDITOR</h1>
    <form action="{{ route('edit.done',$edit->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="container mt-10 w-full flex flex-col items-center">
            <div class="m-3 first-row flex-col md:flex-row flex justify-evenly items-center w-full">
                ...
                <input type="text" name="category" id="category" value="{{ $edit->category }}" />
                <label for="author" class="font-extrabold">AUTHOR</label>
                <input type="text" name="author" id="author" value="{{ $edit->author }}" />
            </div>
            <br />
            <div class="m-3 w-full second-row flex-col md:flex-row flex justify-evenly items-center">
                ...
                <input type="text" name="keywords" id="keywords" value="{{ $edit->keywords }}" />
                ...
                <input type="text" name="credit" id="credit" value="{{ $edit->credit }}"/>
            </div>
        </div>
